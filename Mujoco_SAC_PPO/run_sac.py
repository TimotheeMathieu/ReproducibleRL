...
def run_seed(seed):
    ...
    if args.pretrained:
        log_dir = f"{output_name}/sac_logs/{env}_pretrained/{hostname}"
    else:
        log_dir = f"{output_name}/sac_logs/{env}/{hostname}"
    # Train the model on a few steps
    model = SAC("MlpPolicy", env, verbose=0, seed=seed, device='cpu', tensorboard_log=log_dir)
    if args.pretrained:
        n_steps = 50_000
        model.load("../Mujoco/pretrain_model/sac-Hopper-v3.zip")
    else:
        n_steps = 1_000_000
